...
import { useGetAnalyticsDriversAverageTime } from '../../../api/analytics/analytics/analytics';
import type { ModelDriverAverageTimeResponse } from '../../../api/analytics/model';
...
type DriverAverageTimeWidgetProps = {
  title?: string;
  xAxisLabel?: string;
  yAxisLabel?: string;
};

export const DriverAverageTimeWidget = ({
  title = 'Среднее время выполнения заказов водителями',
  xAxisLabel = 'Водитель',
  yAxisLabel = 'Время (минуты)',
}: DriverAverageTimeWidgetProps) => {
  ...
  const driverData: ModelDriverAverageTimeResponse[] = useMemo(() => {
    if (!apiData) return [];
    return apiData;
  }, [apiData]);
  ...
  const stackedBarData = useMemo(() => {
    return driverData
      ...
      .map((driver) => ({
        driverName: driver.driverName || 'Неизвестный водитель',
        cargoTypes: {
          'Среднее время (мин)': Math.round((driver.averageTimeMinutes || 0) * 10) / 10, // Округляем до 1 знака после запятой
        },
      }));
  }, [driverData]);
